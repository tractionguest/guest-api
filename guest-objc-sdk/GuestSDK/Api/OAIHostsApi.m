...
@interface OAIHostsApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation OAIHostsApi

NSString* kOAIHostsApiErrorDomain = @"OAIHostsApiErrorDomain";
NSInteger kOAIHostsApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) createHostWithHost: (OAIHost*) host
    ...
    if (host == nil) {
        ...
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"host"] };
            NSError* error = [NSError errorWithDomain:kOAIHostsApiErrorDomain code:kOAIHostsApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"OAIHost*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((OAIHost*)data, error);
                                }
                            }];
}
...
-(NSURLSessionTask*) getHostsWithQuery: (NSString*) query
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"OAIPaginatedHostsList*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((OAIPaginatedHostsList*)data, error);
                                }
                            }];
}
